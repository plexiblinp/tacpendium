-- 000072_m20_seed_aliases_numeric.up.sql
-- M20-02: 表記プリセット numeric のエイリアス投入(D-311 / D-314 / D-315 / D-321)
...
SELECT (SELECT id FROM presets WHERE code = 'numeric'), m.id, v.alias_text, v.alias_text_en
FROM moves m
...
SELECT (SELECT id FROM presets WHERE code = 'numeric'), m.id, v.alias_text
FROM moves m
JOIN characters c ON c.id = m.character_id
CROSS JOIN (
