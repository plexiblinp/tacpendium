...
// M24-09c §4.3: 干渉を決定論的に起こす probe（2 本で 1 組。片割れは aa-interference-probe-a.spec.ts）。
//
...
test.describe("M24-09c 干渉プローブ B", () => {
  test("B: 同じ識別キーの本登録コンボを他ファイルと取り合っていない", async ({ request }) => {
    await runInterferenceProbe(request, "B");
  });
});
