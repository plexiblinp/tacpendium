...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      {/* ★モーダルが開いている間、物理入力は受け手の面の状態を変えない（`DES-005` §6.4.3
          項目 4″）。抑止の軸そのものは `PhysicalInputProvider` に 1 本だけあり、ここは
          「開いている」ことを申告するだけである。**本コンポーネントを通るモーダルは、
          足した人が何もしなくても対象になる**（自作のオーバーレイは自分で目印を置く必要が
          ある＝`lib/modal-presence.ts` の注記）。
          ★Portal の内側（＝開閉で入れ替わる側）に置くこと。ラッパ本体で申告すると、
          画面に置いてあるだけのダイアログが常に「開いている」ことになる。 */}
      <ModalPresenceMarker />
      {children}
